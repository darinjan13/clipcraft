...
const mode = inspect.Config.Env.find((value) => value.startsWith('TEXT_EXECUTION_MODE='))?.split('=', 2)[1] || 'unset';
...
function summarize(value) {
  ...
  for (const [key, item] of Object.entries(value)) {
    if (/secret|token|authorization|prompt/i.test(key)) result[key] = '<redacted>';
    else result[key] = summarize(item);
  }
  return result;
}

async function main() {
  const path = 'phase7-wf17-controlled-probe';
  const workflow = {
    name: `Phase 7 WF17 Controlled Probe ${Date.now()}`,
    nodes: [
      {
        ...
        typeVersion: 2.1,
        position: [0, 0],
        id: 'phase7-probe-webhook-000000000001',
        name: 'Controlled Probe Webhook',
      },
      {
        parameters: { jsCode: "const body = $json.body && typeof $json.body === 'object' ? $json.body : $json; return [{ json: body }];" },
        type: 'n8n-nodes-base.code',
        typeVersion: 2,
        position: [180, 0],
        id: 'phase7-probe-unwrapper-000000000002',
        name: 'Unwrap Probe Input',
      },
      {
        parameters: { workflowId: { __rl: true, value: '17', mode: 'list', cachedResultName: 'AI Generate Text' }, mode: 'each' },
        type: 'n8n-nodes-base.executeWorkflow',
        typeVersion: 2,
        position: [440, 0],
        id: 'phase7-probe-execute-000000000003',
        name: 'Execute WF17',
      },
    ],
    connections: {
      'Controlled Probe Webhook': { main: [[{ node: 'Unwrap Probe Input', type: 'main', index: 0 }]] },
      'Unwrap Probe Input': { main: [[{ node: 'Execute WF17', type: 'main', index: 0 }]] },
    },
  ...
  try {
    ...
    const input = {
      ...
      prompt: 'Write one short sentence about calm focus.',
      systemPrompt: 'Return only one sentence.',
      provider: 'cloudflare',
      modelId: '@cf/meta/llama-3.1-8b-instruct',
      _testCorrelationId: 'phase7-controlled-internal-text',
    };
    const response = await fetch(`http://localhost:5680/webhook/${path}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) });
    const text = await response.text();
